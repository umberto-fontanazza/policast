...
fn list_screen_capture_devices_macos() -> io::Result<HashMap<String, String>> {
    ...
    if devices.is_empty() {
        Err(io::Error::new(io::ErrorKind::NotFound, "No screen capture devices found"))
    } else {
        Ok(devices)
    }
}
...
pub fn list_screen_capture_devices() -> io::Result<HashMap<String, String>> {
    if cfg!(target_os = "macos") {
        ...
        Err(io::Error::new(io::ErrorKind::Other, "Unsupported operating system"))
    }
}


fn get_ffmpeg_args(video_width: u32, video_height: u32, x: u32, y: u32, target: &str) -> Vec<String> {
...
pub fn start_screen_capture(video_width: u32, video_height: u32, x: u32, y: u32, target: &str) -> io::Result<Child> {
